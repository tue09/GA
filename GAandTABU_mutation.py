...
def Genetic_Algorithm(current_population, tournament_size, crossover_rate, mutation_rate, number_iteration):
    ...
    for i in range(number_iteration):
        ...
        for j in range(int(len(current_population)/2)):
            ...
            #Mutation:
            child1, child2 = TABU.tabu_search(file_path, child1[1], 1), TABU.tabu_search(file_path, child2[1], 1)
    ...
    return best
read_data(file_path)
"""solution = TABU.tabu_search(file_path, Greedy_procedure(distance_matrix)[1], 30)"""


solution = Genetic_Algorithm(initial_population(min(50, number_of_cities)), 4, 0.95, 1/number_of_cities, 20)
"""array = []
GAP_array = []
for i in range(10):
    print(i)
    solution = Genetic_Algorithm(initial_population(min(50, number_of_cities)), 4, 0.95, 1/number_of_cities, 100)
    GAP = ((solution[0] - 33523) / 33523) * 100
